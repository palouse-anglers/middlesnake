...
get_swifd_by_county <- function(county_names, save_path = NULL) {
  ...
  url <- "https://geodataservices.wdfw.wa.gov/arcgis/rest/services/MapServices/SWIFD/MapServer/0/query?where=1=1&outFields=*&returnGeometry=true&f=geojson"

  resp <- httr::GET(url)
  if (httr::status_code(resp) != 200)
    cli::cli_abort("Failed to download SWIFD data. Status code: {httr::status_code(resp)}")

  geojson_text <- httr::content(resp, as = "text", encoding = "UTF-8")
  swifd_all <- geojsonsf::geojson_sf(geojson_text)

  # Step 3: Validate, reproject
  ...
  # Step 4: Buffer and clip
  buffer_geom <- sf::st_union(sf::st_buffer(matched, 0.0005))
  cli::cli_alert_info("Clipping SWIFD features to selected counties...")
  clipped <- sf::st_intersection(swifd_all, buffer_geom)
  ...
  if (!is.null(save_path)) {
    ...
    cli::cli_alert_success("Saved merged shapefile to {.file {shp_file}}")
  }

  cli::cli_alert_success("Returned {.strong {nrow(clipped)}} SWIFD features across {.val {length(county_names)}} counties.")
  return(clipped)
}
